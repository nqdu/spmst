...
def load_topo(topo_file):
    topo = np.loadtxt(topo_file,skiprows=3)
    f = open(topo_file,"r")
    line = f.readline()
    nx,nz = map(lambda x:int(x),line.split())
    line = f.readline()
    xmin,xmax = map(lambda x:float(x),line.split())
    line = f.readline()
    zmin,zmax = map(lambda x:float(x),line.split())
    n = nz * nx
...
def main():
    if len(sys.argv) != 8:
        print("Usage: ./this x1 y1 x2 y2 zmax toponame model_name")
        exit(1)
    x1,y1,x2,y2,zmax = map(lambda x:float(x),sys.argv[1:6])
    topo_name = sys.argv[6]
    model_name = sys.argv[7]

    # load topography 
    xtopo,ztopo,topo = load_topo(topo_name)
    
    # get spherical flag
    is_spherical = 0
    f = open("velocinit.in","r")
    lines = f.readlines()
    f.close()
    n0 = 0
    for line in lines:
    ...
    # get topography on this profile
    prof_topo = griddata((xtopo,ztopo),topo,(profx,profy))
    f = open("out_topo.txt","w")
    for i in range(n):
        f.write("%f %f\n"%(dist[i],prof_topo[i]))
    f.close()
    ...
    for i in range(n):
        for j in range(n):
            ...
            cord[id,0] = profx[i]
            cord[id,1] = profy[i]
            cord[id,2] = prof_topo[i] + (zmax - prof_topo[i]) / (n-1) * j
